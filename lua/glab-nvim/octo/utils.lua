...
function M.in_pr_repo()
    ...
    if buffer.node.baseRepository.nameWithOwner ~= local_repo then
        vim.notify(
            string.format(
                "[Octo] Not in PR repo. Expected %s, got %s",
                buffer.node.baseRepository.nameWithOwner,
                local_repo
            ),
            2
        )
...
function M.get_comment_at_line(bufnr, line)
    ...
    for _, comment in ipairs(buffer.commentsMetadata) do
        local mark =
            vim.api.nvim_buf_get_extmark_by_id(bufnr, constants.OCTO_COMMENT_NS, comment.extmark, {details = true})
...
function M.get_body_at_cursor(bufnr)
    ...
    local mark =
        vim.api.nvim_buf_get_extmark_by_id(bufnr, constants.OCTO_COMMENT_NS, metadata.extmark, {details = true})
    local start_line = mark[1] + 1
    local end_line = mark[3]["end_row"] + 1
